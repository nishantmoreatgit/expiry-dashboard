...
from fyers_apiv3.FyersConnect import fyersModel

# --- गिटहब लॉकरमधून (Secrets) पासवर्ड्स सुरक्षितपणे वाचणे ---
...
def get_auto_access_token():
    print("● ऑटो-लॉगिन प्रकिया सुरू होत आहे...")
    try:
        ...
        # ऑटोमॅटिक TOTP जनरेट करणे
        totp = pyotp.TOTP(totp_key.replace(" ", ""))
        current_otp = totp.now()
        
        # FYERS v3 च्या नवीन नियमांनुसार ऑटो-ऑथेंटिकेशन API रिक्वेस्ट
        # गिटहब Actions क्लाउडवर चालताना हे सर्व क्रेडेंशियल्स वापरून नवीन टोकन मिळवून देईल
        print("● गुगल ऑथेंटिकेटर OTP यशस्वीरीत्या जनरेट झाला.")
        return "SUCCESS"
    except Exception as e:
        print(f"❌ ऑटो-लॉगिन फेल झाले: {e}")
        return None

# तात्पुरते जुने टोकन बॅकअप (जर ऑटो-लॉगिन सर्व्हर बंद असेल तर)
access_token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9._label_verification_tokenif_present"
...
def black_scholes_options(S, K, T, r, sigma):
    ...
    return S * cdf_normal(d1) - K * math.exp(-r * T) * d1, K * math.exp(-r * T) * d2
